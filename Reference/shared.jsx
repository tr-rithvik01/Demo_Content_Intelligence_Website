...
function NewsletterSignup({ compact = false }) {
  ...
  if (state === 'success') {
    return (
      <div className="newsletter-box" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
        ...
        <div style={{ fontSize: 18, fontWeight: 800 }}>You're on the list.</div>
        <div style={{ fontSize: 14, color: 'var(--text-2)', maxWidth: 320, textAlign: 'center' }}>
          Welcome to The AI GTM Engineer. Check your inbox for a confirmation, then watch for your first edition on Thursday.
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="newsletter-box">
      <div className="newsletter-eyebrow">Weekly Intelligence</div>
      <h2 className="newsletter-title">The signal through the noise.</h2>
      <p className="newsletter-sub">
        Every Thursday: one tactic, one framework, one data point. For marketers who prefer evidence over opinion.
      </p>
      <form className="newsletter-form" onSubmit={handleSubmit} noValidate>
        <input
          type="email"
          className={`newsletter-input${state === 'error' ? ' form-error' : ''}`}
      ...
      {state === 'error' && <div className="form-error" style={{ marginTop: 8, textAlign: 'center' }}>Please enter a valid email address.</div>}
