...
function Button({ variant = 'primary', size = '', children, onClick, disabled, loading, className = '' }) {
  ...
  return (
    <button className={cls} onClick={onClick} disabled={disabled || loading} aria-busy={loading}>
...
function TagBadge({ label, color = 'neutral', onClick }) {
  return (
    <span className={`tag tag-${color}`} onClick={onClick} role={onClick ? 'button' : undefined}>
      {label}
    </span>
  );
}
...
function ImgPlaceholder({ label, height = 180, style = {} }) {
  return (
    <div className="article-card-img" style={{ height, ...style }}>
      <div className="article-card-stripe"></div>
      <span className="article-card-img-label font-mono">{label}</span>
    </div>
  );
}
    ...
    <a href={`/blog/${article.slug}`} className="article-card fade-in" aria-label={article.title} style={{ textDecoration: 'none', color: 'inherit', display: 'flex' }}>
      ...
      <div className="article-card-body">
        <div className="article-card-meta">
          <TagBadge label={article.niche} color={tagColor} />
          {article.industry !== 'All Industries' && <TagBadge label={article.industry} color="neutral" />}
        </div>
    ...
    <a href={`/blog/${article.slug}`} className="article-card-featured" style={{ textDecoration: 'none', color: 'inherit' }}>
      <div className="article-card-featured-body">
        <div style={{ display: 'flex', gap: 8, marginBottom: 4 }}>
          ...
          <TagBadge label={article.niche} color="green" />
        </div>
        <h2 className="article-card-featured-title">{article.title}</h2>
        <p style={{ fontSize: 15, color: 'var(--text-2)', lineHeight: 1.6 }}>{article.excerpt}</p>
        <div className="article-author" style={{ marginTop: 8 }}>
          <div className="author-avatar">{article.author.initials}</div>
          <div>
            <div style={{ fontSize: 13, fontWeight: 600 }}>{article.author.name}</div>
            <div style={{ fontSize: 12, color: 'var(--text-3)' }}>{article.date} · {article.readTime} read</div>
          </div>
        </div>
...
function NicheCard({ niche }) {
  return (
    <a href={`/niches/${niche.slug || niche.id}`} className="niche-card" style={{ textDecoration: 'none', color: 'inherit' }}>
      ...
      <div>
        <div className="niche-card-name">{niche.name}</div>
        <div className="niche-card-count">{niche.count} guides</div>
      </div>
...
function CalloutBox({ type = 'tip', title, children }) {
  ...
  return (
    <div className={`callout callout-${type}`} role="note">
      <span className="callout-icon" aria-hidden="true" style={{ fontFamily: 'var(--font-mono)', fontWeight: 700 }}>{marks[type]}</span>
      <div>
...
function NewsletterSignup() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!value || !validate()) { setState('error'); return; }
    setState('loading');
    setTimeout(() => setState('success'), 1400);
  };

  const handleModeSwitch = (m) => { setMode(m); setValue(''); setState('idle'); };

  if (state === 'success') {
    return (
      <div className="newsletter-box" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
        <div style={{ width: 44, height: 44, borderRadius: 'var(--r-lg)', background: 'var(--green-sub)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M4 10l4.5 4.5 7.5-9" stroke="var(--green)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
        </div>
        <div style={{ fontSize: 18, fontWeight: 800 }}>You're in.</div>
        <div style={{ fontSize: 14, color: 'var(--text-2)', maxWidth: 340, textAlign: 'center' }}>
          Welcome to GTM Remixed. Expect the good stuff — no filler, no fluff. Just what's actually moving the needle in your industry, straight to you.
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="newsletter-box">
      ...
      <h2 className="newsletter-title">
        There's a lot happening out there.<br />
        <span style={{ color: 'var(--accent)', fontStyle: 'italic', fontFamily: 'var(--font-serif)' }}>What matters is your work.</span>
      </h2>
      <p className="newsletter-sub">
        Guides, tips, updates, and hacks — delivered straight to your inbox so you don't have to go looking for them. No noise. Just the stuff worth your time.
      </p>
      ...
      <form className="newsletter-form" onSubmit={handleSubmit} noValidate>
        ...
        <Button variant="primary" loading={state === 'loading'}>
          {state === 'loading' ? '' : 'Subscribe'}
        </Button>
      </form>
      {state === 'error' && (
        <div className="form-error" style={{ marginTop: 8, textAlign: 'center' }}>
          {mode === 'email' ? 'Please enter a valid email address.' : 'Please enter a valid phone number.'}
        </div>
      )}
      <p className="newsletter-proof">14,200+ marketers. No spam. Unsubscribe anytime.</p>
    </div>
  );
}
  ...
  return (
    <div className="code-block">
      <div className="code-block-header">
        <span>{lang}</span>
        <button className="btn btn-ghost btn-sm" onClick={handleCopy} style={{ fontSize: 12 }}>
          {copied ? '✓ Copied' : 'Copy'}
        </button>
      </div>
...
function ShareBar() {
  ...
  return (
    <div className="share-bar">
      <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-3)' }}>Share:</span>
      <button className="share-btn">X / Twitter</button>
      <button className="share-btn">LinkedIn</button>
      <button className="share-btn" onClick={handleCopy}>{copied ? 'Link copied' : 'Copy link'}</button>
    </div>
  );
}
...
function FAQItem({ question, answer }) {
  ...
  return (
    <div className="faq-item">
      <div className={`faq-q${open ? ' open' : ''}`} onClick={() => setOpen(!open)} role="button" tabIndex={0} onKeyDown={e => e.key === 'Enter' && setOpen(!open)} aria-expanded={open}>
        <span>{question}</span>
      ...
      {open && <div className="faq-a fade-in">{answer}</div>}
    </div>
  );
}
...
function useScrollAppear(ref) {
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(([entry]) => {
      if (entry.isIntersecting) { ref.current.classList.add('visible'); obs.disconnect(); }
    }, { threshold: 0.1 });
...
function AppearSection({ children, style }) {
  ...
  useScrollAppear(ref);
  return <div className="appear" ref={ref} style={style}>{children}</div>;
}
